...
with tab1:
    ...
    if audio is not None:
        ...
        if choice == "Pre-Recorded Audio":
            ...
            "Confidence Score Test #1": "Units arrived at a residence on Elm Street after reports of a burning smell in the basement. Light haze visible upon entry but no active flames. Power was shut off as a precaution and crews checked walls for heat using a thermal camera. Possible cause believed to be an overheated appliance. No injuries reported. Weather was windy with light rain. Engine 204 and Ladder 112 on scene for about 40 minutes."
            }
...
with tab2:
    ...
    # Initialize persistent storage for parsed data
    if "parsed" not in st.session_state:
        st.info("No parsed incident yet. Go to 'Record / Input' tab first.")
    else:
        # 🧠 Preserve parsed data across reruns
        if "approved_parsed" not in st.session_state:
            st.session_state["approved_parsed"] = st.session_state["parsed"]
        ...
        st.session_state["approved_parsed"] = parsed
